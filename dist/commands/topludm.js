    ...
    async execute(interaction) {
        ...
        if (logChannel) {
            const logEmbed = new discord_js_1.EmbedBuilder()
                ...
                .setColor('#2F3136')
                .addFields({ name: 'Gönderen Yetkili', value: `${interaction.user}`, inline: true }, { name: 'Başarılı', value: `${basarili} kişi`, inline: true }, { name: 'Başarısız', value: `${basarisiz} kişi`, inline: true }, { name: 'Gönderilen Mesaj', value: `\`\`\`\n${message.substring(0, 1000)}\n\`\`\``, inline: false })
                .setTimestamp();
